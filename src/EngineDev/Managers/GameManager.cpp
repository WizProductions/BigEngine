...
GETTER float GameManager::GetDeltaTime() const { return m_Timer->DeltaTime(); }
...
void GameManager::Update() {
	...
	for (const auto entityToSpawn : m_EntitiesPendingSpawn) {
		lastEntityIndex++;
		CheckEntityIndexBounds(); // Vérification des limites
...
void GameManager::CheckEntityIndexBounds() const {
	//(lastEntityIndex >= -1 && lastEntityIndex < entityArraySize && "lastEntityIndex out of bounds");
}
